...
pat <-  "^5_known_overlap_counts.csv$"
dat <- "ko"
for (i in 1:length(dirs)) {
  wd <- dirs[i]
  files <- list.files(path=wd, pattern=pat, full.names=TRUE) 
...
# SAVE PLOTS
name1 <- paste0(paste('worst-case','counts', sep="_"),'.png')
...
# PLOT D - Search Space Size, rank 1
  ...
  corr_rank <- cor.test(r1_mindcf_counts$x1, r1_mindcf_counts$FA, method=c("pearson"))
  
  plot_a <- r1_mindcf_counts %>%
        select(x1, TA, FA) %>%
        pivot_longer(-x1) %>%
      ggplot(aes(x1, value, color=name))+
      geom_point(position=position_jitterdodge(jitter.width=425000,jitter.height=0.1, dodge.width=850000),
               ...
               shape=21)+
      #geom_abline(intercept=lm_total_coef[1], slope = lm_total_coef[2], alpha=0.4, linetype = "dashed")+
      geom_errorbarh(data = r1_mindcf_counts %>% select(x1,TA,FA)%>% pivot_longer(-x1)%>%group_by(x1,name)%>%summarise(value=mean(value)), aes(xmin=x1-425000,xmax=x1+425000,group=name), stat=,position=position_dodge(width=850000),height=0,size=2)+
        theme_classic()+
  ...
  plot_a <- plot_a + geom_text(x=-Inf, y=6, label='FA ~ comparisons', hjust=-.1, yjust=0, size=3, color="#000000", show.legend = FALSE) + geom_text(x=-Inf, y=5.7, label=paste0("r = ",as.character(signif(corr_rank$estimate, digits=4))), hjust=-.2, yjust=0, size=3, show.legend = FALSE) + geom_text(x=-Inf, y=5.4, label=paste0("p-value < 2.2e-16"), hjust=-.1, yjust=0, size=3,show.legend = FALSE)
...
# arrange plots
t <- ggarrange(plot_a,plot_b, ncol=2, nrow=1, common.legend = TRUE, legend="bottom")

# SAVE PLOTS
name1 <- paste0(paste('rank1','mindcf', sep="_"),'.png')
